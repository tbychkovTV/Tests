...
and the execution of order can only be detected indirectly in a script by analyzing changes in built-in variables such as `strategy.opentrades <https://www.tradingview.com/pine-script-reference/v4/#var_strategy{dot}opentrades>`__. 
*Script alert* configured on *order fill events* are thus useful in that they allow the triggering of alerts at the precise moment an order is executed, 
before a script's logic can detect that an order was executed.

While it is not required for *order fill events* to trigger correctly, Pine coders can customize the alert message sent when specific orders are executed. 
This is done by means of the `strategy.opentrades <https://www.tradingview.com/pine-script-reference/v4/#var_strategy{dot}opentrades>`__ parameter in functions that 
