...
However, when a *script alert* is created from a strategy, it can also be configured by the user to trigger on *order fill events*. 
An order fill event is any event generated by the broker emulator which causes an order to be executed, 
so the equivalent of a trade order being executed by your broker/exchange. Orders are not necessarily executed when they are placed.
