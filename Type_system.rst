...
The hierarchy of forms entails that whenever a "series" form is required, you may also use "const", "input" or "simple" forms. When a "const" form is required, however, only that form is allowed. Furthermore, once a variable acquires a form, that state is irreversible; it can never be converted back to a weaker form. A variable of "series" form can thus never be converted back to a "simple" form, for use with a function that requires arguments of that form.
